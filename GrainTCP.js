const CFG = { id: 'dd052c98-bd4a-4c04-a0a7-bfd2fa9f5001', chunk: 64 * 1024, dnPack: 32 * 1024, dnTail: 512, dnMs: 0, upPack: 16 * 1024, upQMax: 256 * 1024, maxED: 8 * 1024, concur: 4 };
...
const ws = async req => {
  ...
  const thresh = async () => { if (busy || closed) return; busy = true; try { for (;;) {
    if (closed) break; if (!sock) {
      const [d] = uq.bundle(); if (!d) break;
      
      // 如果存在 pathTarget，则直接使用，忽略 VLESS
      let host, port, payload;
      if (pathTarget) {
        host = pathTarget.host;
        port = pathTarget.port;
        payload = d;  // 整个数据作为 payload
        server.send(new Uint8Array([0])); // 简单回应
      } else {
        const r = vless(d);
        if (!r) throw wither();
